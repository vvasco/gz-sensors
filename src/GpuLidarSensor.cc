...
public:
  gz::common::EventT<void(const float *_scan, unsigned int _width,
...
bool GpuLidarSensor::Load(const sdf::Sensor &_sdf) {
  ...
  msgs::InitPointCloudPacked(
      this->dataPtr->pointMsg, this->Name(), true,
  ...
  // trick to check if the sensor has to be triggered (trigger is not exposed for Lidar in sdf)
...
bool GpuLidarSensor::CreateLidar() {
  ...
  this->dataPtr->pointMsg.set_row_step(this->dataPtr->pointMsg.point_step() *
                                       this->dataPtr->pointMsg.width());
  this->dataPtr->gpuRays->SetVisibilityMask(this->VisibilityMask());

  this->dataPtr->lidarFrameConnection =
      this->dataPtr->gpuRays->ConnectNewGpuRaysFrame(std::bind(
          &GpuLidarSensor::OnNewLidarFrame, this, std::placeholders::_1,
...
/////////////////////////////////////////////////
void GpuLidarSensor::OnNewLidarFrame(const float *_scan, unsigned int _width,
...
bool GpuLidarSensor::Update(const std::chrono::steady_clock::duration &_now) {
  ...
  if (this->dataPtr->pointPub.HasConnections()) {
    ...
    *this->dataPtr->pointMsg.mutable_header()->mutable_stamp() =
        msgs::Convert(_now);
    // Set frame_id
    for (auto i = 0; i < this->dataPtr->pointMsg.mutable_header()->data_size();
         ++i) {
      if (this->dataPtr->pointMsg.mutable_header()->data(i).key() ==
              "frame_id" &&
...
void GpuLidarSensorPrivate::FillPointCloudMsg(const float *_laserBuffer) {
  ...
  float verticleAngleStep =
      (this->gpuRays->VerticalAngleMax() - this->gpuRays->VerticalAngleMin())
          .Radian() /
  ...
  for (uint32_t j = 0; j < height; ++j) {
    ...
    for (uint32_t i = 0; i < width; ++i) {
      ...
      *reinterpret_cast<uint16_t *>(
          msgBufferIndex + this->pointMsg.field(fieldIndex++).offset()) = ring;
